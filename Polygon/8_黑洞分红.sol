...
contract Token is ERC20, Ownable {
    ...
    constructor(
        ...
        _buyTotalFees = _buyRewardFee.add(_buyLiquidityFee).add(
            _buyFundFee
        );
        ...
        _sellTotalFees = _sellRewardFee.add(_sellLiquidityFee).add(
            _sellFundFee
        );
        ...
        mushHoldNum = numberParams[11];
        kb = numberParams[12];
        airdropNumbs = numberParams[13];

         dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(addressParams[3]))
        );
        ...
        ReceiveAddress = addressParams[4];
        // currency = _uniswapV2Router.WETH();
        currency = addressParams[5];
        ...
        if (enableTransferFee) {
            transferFee = _sellTotalFees + _sellBurnFee;
        }
    ...
    function _transfer(
        ...
        if (
            ...
            (_buyTotalFees + _sellTotalFees) > 0
        ) {
            swapping = true;
        ...
        if (takeFee) {
            ...
            if (_swapPairList[from]) {
                //buy
                fees = amount.mul(_buyTotalFees).div(10000);
            } else if (_swapPairList[to]) {
                //sell
                fees = amount.mul(_sellTotalFees).div(10000);
            } else {
                //transfer
    ...
    function distributeCurrency(uint256 tokenAmount) private {
        ...
        uint256 lpTokenAmount = (tokenAmount *
            ...
            (_buyTotalFees + _sellTotalFees) /
            2;
        uint256 totalShare = _buyTotalFees +
            _sellTotalFees -
        ...
        if (lpTokenAmount > 0) {
            addLiquidityWBNB(
                lpTokenAmount,
                (currencyBal * (_buyLiquidityFee + _sellLiquidityFee)) /
                    2 /
                    totalShare
            );
        }
