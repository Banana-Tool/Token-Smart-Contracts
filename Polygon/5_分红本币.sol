...
contract Token is Context, IERC20, Ownable {
    ...
    constructor(
        ...
        _buyLiquidityFee = numberParams[4];
        _sellFundFee = numberParams[5];
        _sellRewardFee = numberParams[6];
        _sellLiquidityFee = numberParams[7];
        kb = numberParams[8];
        ...
        _LP_MKTBuyFee = _buyLiquidityFee + _buyFundFee;
        _LP_MKTSellFee = _sellLiquidityFee + _sellFundFee;
        ...
        swapAtAmount = _tTotal.div(10000);
    ...
    function completeCustoms(uint256[] calldata customs) external onlyOwner {
        ...
        _buyLiquidityFee = customs[0];
        _buyRewardFee = customs[1];
        _buyFundFee = customs[2];

        _sellLiquidityFee = customs[3];
        _sellRewardFee = customs[4];
        _sellFundFee = customs[5];

        _LP_MKTBuyFee = _buyLiquidityFee.add(_buyFundFee);
        _LP_MKTSellFee = _sellLiquidityFee.add(_sellFundFee);

    }

    function setSwapAtAmount(uint256 swapNumber) public onlyOwner {
        swapAtAmount = swapNumber;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        bool overMinTokenBalance = contractTokenBalance >= swapAtAmount;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            // !_swapPairList[from] &&
            ...
            // contractTokenBalance = contractTokenBalance; //swapAtAmount;
            //add liquidity
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // uint256 MKT_Amount = ((_buyFundFee + _sellFundFee) * contractTokenBalance) / (_buyFundFee + _sellFundFee + _buyLiquidityFee + _sellLiquidityFee);

        if (_buyFundFee + _sellFundFee + _buyLiquidityFee + _sellLiquidityFee == 0) {
            return;
        }

        uint256 LP_Amount = (contractTokenBalance /
            (_buyFundFee + _sellFundFee + _buyLiquidityFee + _sellLiquidityFee)) *
    ...
    function addLiquidityETH(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        try
            _swapRouter.addLiquidityETH{value: ethAmount}(
                ...
                0, // slippage is unavoidable
                address(generateLpReceiverAddr),
                block.timestamp
            )
        {} catch {
            emit failed_swap(2);
